...
class energy_efficiency_IBSched(Agent):
    def __init__(
        self,
        env: MARLCommEnv,
        max_number_ues: int,
    ...
    def action_format(
        ...
        if (
            ...
            toggle_flag = action["player_0"][self.max_number_slices]
            scale = (toggle_flag+ 1) / 2
            action["player_0"] = action["player_0"][:self.max_number_slices]
            action["player_0"][
                np.where(
            ...
            rbs_per_slice = (
                scores_to_rbs(
                    action["player_0"],
                    np.floor(
                        self.num_available_rbs[0] / self.rbs_per_rbg * scale).astype(int),
                    self.last_unformatted_obs[0]["basestation_slice_assoc"][
                        0, :
                    ],
                )
        ...
        action_space = spaces.Dict(
            {
                f"player_{idx}": (
                    spaces.Box(
                        low=-1,
                        high=1,
                        shape=(self.max_number_slices + 1,),
                        dtype=np.float64,
                    )
                    if idx == 0
